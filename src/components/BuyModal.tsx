...
import { X, Lock, Unlock, Play, CheckCircle2, XCircle, RotateCcw, Brain, User, Signal, Coins, DollarSign, Info } from 'lucide-react'
...
export default function BuyModal({ pick, onClose }: BuyModalProps) {
  ...
  const refundAmount = (pick.price * 0.95).toFixed(2)
      ...
      <div className="card rounded-xl max-w-md w-full overflow-hidden relative" style={{ borderColor: 'rgba(34,197,94,0.25)' }}>
        ...
        {step === 1 && (
          <div className="p-6 relative">
            ...
            <p className="text-xs mb-5 ml-4" style={{ color: 'var(--muted)' }}>Held in escrow until the game settles.</p>
            ...
            <div className="space-y-2.5 text-sm mb-5">
              ...
              {pick.tier === 'insured' && (
                <div className="flex justify-between">
                  <span className="flex items-center gap-1.5" style={{ color: 'var(--muted)' }}><Coins size={13} />Seller stake</span>
                  <span className="stat-num" style={{ color: 'var(--accent)' }}>${pick.stake ?? 0}</span>
                </div>
              )}
              <div className="flex justify-between">
                <span className="flex items-center gap-1.5" style={{ color: 'var(--muted)' }}><DollarSign size={13} />Price</span>
                <span className="stat-num">${pick.price.toFixed(2)}</span>
              </div>
              {pick.tier === 'insured' && (
                <div className="flex justify-between" style={{ color: 'var(--accent)' }}>
                  <span className="flex items-center gap-1.5"><RotateCcw size={13} />Refund if loses</span>
                  <span className="stat-num">${refundAmount} (95%)</span>
                </div>
              )}
              <hr style={{ borderColor: 'var(--border)' }} />
              <div className="flex justify-between font-semibold text-base">
                <span>Charge now</span>
                <span className="stat-num text-gradient">${pick.price.toFixed(2)}</span>
              </div>
            </div>
            ...
            <div className="rounded-md p-3 text-[11px] mb-5 flex items-start gap-2" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border)', color: 'var(--muted)' }}>
              ...
              <span>Held in escrow until game settles. Auto-refund processed within 1h of game end if pick loses. Powered by official box scores.</span>
            </div>

            <div className="flex gap-2">
              <button onClick={onClose} className="btn-ghost px-4 py-3 rounded-md flex-1 text-sm">Cancel</button>
              <button onClick={handleUnlock} className="btn-primary unlock-btn px-4 py-3 rounded-md flex-1 flex items-center justify-center gap-1.5 text-sm">
        ...
        {step === 3 && result && (
          <div className="p-6 text-center relative">
            ...
            <div className="text-sm text-left rounded-md p-4 border mb-6" style={{ background: 'var(--panel2)', borderColor: 'var(--border)', color: 'var(--text)' }}>
              {result === 'won' ? (
                <p>Congratulations! The pick won. Your profit has been added to your account balance.</p>
              ) : (
                <p>The pick lost. Since this was an <strong>insured pick</strong>, you&apos;ve received a 95% auto-refund of <strong>${(pick.price * 0.95).toFixed(2)}</strong> back to your account within 1 hour.</p>
              )}
            </div>
            <div className="flex gap-2">
